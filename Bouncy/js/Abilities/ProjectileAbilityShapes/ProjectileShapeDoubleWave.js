 ...
 */
class ProjectileShapeDoubleWave extends ProjectileShape {
  ...
  doActionOnTick(playerID, tick, boardState, castPoint, targetPoint) {
    ...
    for (let direction = -1; direction <= 1; direction += 2) {
      if (
        ...
        ) + ((this.ANGLE_SPREAD) * pctDone + this.ANGLE_OFFSET) * direction;
    ...
    for (let direction = -1; direction <= 1; direction += 2) {
      for (var i = 0; i <= 1; i+= 1) {
        var angle = Math.atan2(endPos.y - startPos.y, endPos.x - startPos.x) +
          (this.ANGLE_SPREAD * i + this.ANGLE_OFFSET) * direction;
        var dist = 250;
        dist -= circleSize;
