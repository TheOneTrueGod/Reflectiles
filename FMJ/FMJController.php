...
class FMJController {
  ...
  public function getResponse($request, $gameObject, $user) {
    ...
    switch ($action) {
      case FMJ_SERVER_ACTIONS['GET_BOARD_DATA']:
        return $this->getBoardData();
      case FMJ_SERVER_ACTIONS['GET_GAME_METADATA']:
        return $this->getGameMetaData();
      case FMJ_SERVER_ACTIONS['GET_TURN_STATUS']:
        return $this->getTurnStatus();
      case FMJ_SERVER_ACTIONS['SET_BOARD_AT_TURN_START']:
      ...
      case FMJ_SERVER_ACTIONS['FINALIZE_TURN']:
        if (!$this->gameObject->isUserHost($this->user)) {
          throw new Exception("Only the host can do this action");
        }
        return $this->finalizeTurn();
      case FMJ_SERVER_ACTIONS['FINISH_GAME']:
        if (!$this->gameObject->isUserHost($this->user)) {
          throw new Exception("Only the host can do this action");
        }
        return $this->finishGame();
      case FMJ_SERVER_ACTIONS['SUBMIT_PLAYER_COMMANDS']:
        return $this->savePlayerCommands();
      case FMJ_SERVER_ACTIONS['UPDATE_PRE_GAME_STATE']:
        return $this->updatePreGameState();
      break;
    }
    throw new Exception("$action not handled in FMJController");
    return "";
  }
  ...
  private function getGameHTML($user) {
    ...
    if ($this->gameObject->getMetadata()->isGameStarted()) {
      require('FMJGameHTML.php');
    } else {
      require('FMJPageHTML.php');
    }
    ?>
